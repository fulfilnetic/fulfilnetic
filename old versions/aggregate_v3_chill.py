...
- Reads main CSV/XLSX with Fulfilment Fee + Number of Labels
- Reads admin Excel with Storage (col G) and PIM (col I)
- Optional Teamleader template to mirror upload columns
- Validates labels>0, flags rows where fee is 0/NaN, computes fee*labels totals
- Writes: Rows_Used, Totals_by_Seller, Admin_Aggregates, optional Data_Issues + Issue_Summary_By_Seller, optional Teamleader_Upload
"""
...
def aggregate_main(df, seller_col, seller_name_col, fee_col, labels_col):
    ...
    if len(issues)>0:
        ...
        issues=clean_frame(issues); by_seller=clean_frame(by_seller)
    ...
    return clean_frame(totals), clean_frame(df_ok), (issues if len(issues)>0 else None), by_seller
...
def resolve_admin_columns(df, admin_seller_col, storage_col, pim_col):
    ...
    if storage_col is None and len(cols)>=7: storage_col=cols[6]  # col G
    if pim_col is None and len(cols)>=9: pim_col=cols[8]          # col I
...
def merge_totals(totals, admin_agg):
    totals["__key"] = norm_key_series(totals["Seller"])
    admin_agg["__key"] = norm_key_series(admin_agg["Seller"])
    merged = totals.merge(admin_agg.drop(columns=["Seller"]).rename(columns={"__key":"__key_admin"}),
                          left_on="__key", right_on="__key_admin", how="left")
    merged.drop(columns=["__key","__key_admin"], inplace=True)
    # Prefer Seller from totals
    return merged

def main():
    ap = argparse.ArgumentParser(description="Aggregate fulfilment + storage + PIM and build Teamleader upload.")
