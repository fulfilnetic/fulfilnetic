#!/usr/bin/env python3
"""
v3.6 STRICT — Minimal tabs, VLOOKUP mapping, issues tabs, and Seller Name in Admin_Aggregates.

Tabs:
- Rows_Used
- Admin_Aggregates              : now contains Seller + Seller Name (merged from main for overlapping sellers)
- Totals_by_Seller (+ TOTAL)    : Fulfilment + Storage + PIM + Grand Total
- Data_Issues
- Issue_Summary_By_Seller
- Teamleader_Upload (optional)

Join logic:
- Admin_Aggregates is built per Seller from admin file; then we enrich it with Seller Name from main totals.
- VLOOKUP-style mapping adds Storage/PIM onto Totals_by_Seller.
"""
...
def main():
    ap = argparse.ArgumentParser(description="v3.6 strict: VLOOKUP join + Seller Name in Admin_Aggregates + issues tabs")
    ...
    # Enrich Admin_Aggregates with Seller Name for overlapping sellers
    # Use totals as the source of canonical Seller Name
    name_map = pd.Series(totals["Seller Name"].values, index=totals["Seller"].astype(str).str.strip()).to_dict()
    admin_agg = admin_agg.copy()
    admin_agg["Seller Name"] = admin_agg["Seller"].astype(str).str.strip().map(name_map)
