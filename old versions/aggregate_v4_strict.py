#!/usr/bin/env python3
"""
v3.3 STRICT — Aggregate Fulfilment + Storage + PIM per Seller and build Teamleader upload.

New:
- Explicit admin column selection by Excel letter: --seller-letter, --storage-letter, --pim-letter
- TOTAL row appended to Totals_by_Seller
- Join_Diagnostics sheet with unmatched sellers
- Strict join (string cast + strip) only

Usage example:
  python aggregate_fulfilment_fees_v3_3_strict.py \
    --input main.csv \
    --admin "ChannelDock  Administration.xlsx" \
    --template "Import-template-teamleader-oktober.xlsx" \
    --output merged.xlsx \
    --storage-letter G --pim-letter I --seller-letter B \
    --verbose
"""
...
def find_any(df, keys_any):
    for c in df.columns:
        low = str(c).lower()
        if any(k in low for k in keys_any):
            return c
    return None
...
def resolve_admin_columns(df, seller_letter=None, seller_col=None, storage_letter=None, storage_col=None, pim_letter=None, pim_col=None):
    ...
    if pim_letter:
        ...
        pim_col = cols[idx]
    # If still missing, try fuzzy
    low=[c.lower() for c in cols]
    def find_any(preds):
        for i,c in enumerate(low):
            if any(p in c for p in preds): return cols[i]
        return None
    if not seller_col: seller_col = find_any(["seller","customer","client","company","debiteur","klant","name"]) or cols[0]
    if not storage_col: storage_col = find_any(["storage"]) or (cols[6] if len(cols)>=7 else None)
    if not pim_col: pim_col = find_any(["pim"]) or (cols[8] if len(cols)>=9 else None)
    return seller_col, storage_col, pim_col

def aggregate_admin(df, admin_seller_col, storage_col, pim_col):
    df=df.copy(); df.columns=[str(c).strip() for c in df.columns]
    if storage_col and storage_col not in df.columns: df[storage_col]=0.0
    if pim_col and pim_col not in df.columns: df[pim_col]=0.0
    if storage_col: df[storage_col]=df[storage_col].apply(smart_to_float)
    if pim_col: df[pim_col]=df[pim_col].apply(smart_to_float)
    use_cols=[c for c in [storage_col, pim_col] if c]
    grp=(df.groupby(admin_seller_col, dropna=False)[use_cols].sum(min_count=1).reset_index()
...
def append_total_row(df):
    ...
    # Copy non-numeric essential fields if needed
...
def main():
    ap = argparse.ArgumentParser(description="v3.3 strict merge + totals row + diagnostics")
    ...
    ap.add_argument("--seller-letter", default=None, help="Excel column letter for seller in admin (e.g., B)")
    ap.add_argument("--storage-letter", default=None, help="Excel column letter for storage cost in admin (e.g., G)")
    ap.add_argument("--pim-letter", default=None, help="Excel column letter for PIM cost in admin (e.g., I)")
    ...
    admin_seller_col, storage_col, pim_col = resolve_admin_columns(admin_df, args.seller_letter, args.admin_seller_col, args.storage_letter, args.storage_col, args.pim_letter, args.pim_col)
