#!/usr/bin/env python3
"""
v3.5 STRICT — Minimal tabs, VLOOKUP mapping, and restored issue reporting.

Tabs:
- Rows_Used                         : rows used in totals (labels > 0 and fee > 0)
- Admin_Aggregates                  : Storage & PIM per Seller (source: admin workbook)
- Totals_by_Seller (+ TOTAL row)    : Fulfilment + Storage + PIM + Grand Total
- Data_Issues                       : rows with labels > 0 but fee missing or zero
- Issue_Summary_By_Seller           : count of issue rows and total labels affected per Seller
- Teamleader_Upload (optional)      : only if --template provided

Join logic:
- Build Admin_Aggregates per Seller
- VLOOKUP-style map onto Totals_by_Seller using strict keys (strip only)

Exit:
- code 2 if issues exist and --allow-issues not provided
"""
...
def aggregate_main(df, seller_col, seller_name_col, fee_col, labels_col):
    ...
    by_seller=None
    if len(issues)>0:
        issues["__Row__"]=issues.index.astype(int)+2  # approx Excel row number
...
def main():
    ap = argparse.ArgumentParser(description="v3.5 strict: minimal tabs + VLOOKUP + issue tabs")
