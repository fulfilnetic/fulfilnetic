#!/usr/bin/env python3
"""
v3.6.1 STRICT — Admin_Aggregates tweaks:
- Columns order: Seller, Seller Name, Storage Cost, PIM Cost
- Include ALL sellers from main (union with admin). Fill missing Storage/PIM with 0.
- VLOOKUP join to Totals_by_Seller unchanged
- Issue tabs unchanged
"""
...
def main():
    ap = argparse.ArgumentParser(description="v3.6.1 strict: Admin_Aggregates order and union with main sellers")
    ...
    # Union with all sellers from main and place Seller Name as 2nd column
    # Build name map from totals
    name_map = pd.Series(totals["Seller Name"].values, index=totals["Seller"].astype(str).str.strip()).to_dict()
    # Normalized keys
    ...
    # Reindex admin_agg to include all sellers; fill missing costs with 0
    admin_agg_full = (admin_agg.set_index("Seller")
                      .reindex(all_sellers)
                      .fillna({"Storage Cost":0.0, "PIM Cost":0.0})
                      .reset_index().rename(columns={"index":"Seller"}))
    # Add Seller Name as 2nd column
    admin_agg_full["Seller Name"] = admin_agg_full["Seller"].map(name_map)
    # Reorder columns
    ...
    # Append TOTAL row to Totals_by_Seller
    totals_with_total = (lambda df: (df.copy()
                                     .pipe(lambda d: pd.concat([d, pd.DataFrame([{**{c:'' for c in d.columns}, **{c: d.select_dtypes(include=[np.number])[c].sum(numeric_only=True) for c in d.select_dtypes(include=[np.number]).columns}, "Seller": "TOTAL"}])], ignore_index=True))))(totals)
