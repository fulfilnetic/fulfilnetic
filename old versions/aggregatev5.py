#!/usr/bin/env python3
"""
v3.6.3 STRICT — VLOOKUP join + issues + admin-first names + DUMMY ROWS for admin-only sellers.

New:
- Dummy rows for sellers present only in admin so they appear in Totals_by_Seller.
- TOTAL row recalculated after injection.
- Tabs unchanged.

Tabs:
- Rows_Used
- Admin_Aggregates (Seller, Seller Name, Storage Cost, PIM Cost; union of sellers)
- Totals_by_Seller (+ TOTAL)
- Data_Issues
- Issue_Summary_By_Seller
- Teamleader_Upload (optional)
"""
...
def main():
    ap = argparse.ArgumentParser(description="v3.6.3 strict: dummy rows for admin-only sellers")
    ...
    # Inject DUMMY rows for admin-only sellers
    missing_sellers = set(admin_agg_full["Seller"]) - set(totals["Seller"])
    if missing_sellers:
        dummy_rows = admin_agg_full.loc[admin_agg_full["Seller"].isin(missing_sellers), ["Seller","Seller Name","Storage Cost","PIM Cost"]].copy()
    ...
    totals_with_total = (lambda df: (df.copy()
                                     .pipe(lambda d: pd.concat([d, pd.DataFrame([{**{c:'' for c in d.columns},
                                                                                   **{c: d.select_dtypes(include=[np.number])[c].sum(numeric_only=True)
                                                                                      for c in d.select_dtypes(include=[np.number]).columns},
                                                                                   "Seller": "TOTAL"}])], ignore_index=True))))(totals)
