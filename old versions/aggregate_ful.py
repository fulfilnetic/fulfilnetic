...
Flow:
1) Load CSV/XLSX. If CSV, auto-detect delimiter/encoding and write an .xlsx copy.
2) Resolve columns: Seller, Seller Name, Fulfilment Fee, Number of Labels.
3) Parse numbers robustly (€, decimal comma/point).
4) Keep only rows with labels > 0.
5) Build issues where labels > 0 but fee is NaN or 0.
   - Default: write issues sheets and exit with code 2. Use --allow-issues to continue (excluding bad rows).
6) Compute per-row total = fee * labels.
7) Aggregate by Seller (+ Seller Name). Write Excel with sheets:
   - Rows_Used + Totals_by_Seller   OR
   - Data_Issues + Issue_Summary_By_Seller + Rows_With_Labels (if aborting)

Usage:
  python aggregate_fulfilment_fees.py --input "in.csv" --output "out.xlsx" [--allow-issues] [--verbose]
  Optional: --seller-col, --seller-name-col, --fee-col, --labels-col, --encoding, --sheet-name, --csv
"""
...
def load_input(input_path, encoding=None, verbose=False, sheet_name=None):
    ...
    if ext == ".csv":
        ...
        # Save as xlsx first for a stable downstream flow
...
def aggregate(df, seller_col, seller_name_col, fee_col, labels_col, allow_issues=False, verbose=False):
    ...
    if len(issues) > 0 and not allow_issues:
        ...
        return None, df_labels, issues, by_seller

    # Compute totals excluding bad rows
...
def main():
    ...
    ap.add_argument("--allow-issues", action="store_true", help="Continue and exclude bad rows instead of aborting")
    ...
    with pd.ExcelWriter(args.output, engine="xlsxwriter") as writer:
        if issues_rows is not None:
            ...
            df_clean.to_excel(writer, index=False, sheet_name="Rows_With_Labels")
        else:
    ...
    # Exit code to signal issues when present
    if issues_rows is not None:
        log("[result] Issues detected. Fix data or rerun with --allow-issues.", args.verbose)
        sys.exit(2)
