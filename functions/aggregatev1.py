#!/usr/bin/env python3
"""
v3.6.5 STRICT — Header detection fix.
...
- Detect and filter out header rows to prevent treating column headers as seller data.
"""
...
def is_likely_header_value(value, col_name):
    """Check if a value looks like a column header rather than actual data"""
    if pd.isna(value):
        return False
    ...
    header_patterns = ["id", "company", "name", "seller", "fee", "cost", "labels", "number", "total", "amount"]
    
    # If value matches common header patterns
    if val_str in header_patterns:
        return True
    
    # If value is very similar to column name
    if val_str == col_str or val_str in col_str or col_str in val_str:
        return True
    
    # If value is very short and generic
    if len(val_str) <= 3 and val_str in ["id", "co", "nm", "nm", "sl", "fe", "lb", "nr"]:
        return True
    
    return False

def validate_detected_columns(df, seller_col, seller_name_col, fee_col, labels_col):
    """Validate that detected columns contain actual data, not headers"""
    if seller_col and seller_col in df.columns:
        # Check first few non-null values
        sample_values = df[seller_col].dropna().head(5)
        header_count = sum(1 for val in sample_values if is_likely_header_value(val, seller_col))
        
        if header_count >= len(sample_values) * 0.6:  # If 60%+ look like headers
            log(f"Warning: Column '{seller_col}' appears to contain headers rather than seller data", True)
            return False
...
def resolve_main_columns(df, seller_col, seller_name_col, fee_col, labels_col):
    ...
    if not validate_detected_columns(df, seller_col, seller_name_col, fee_col, labels_col):
        raise RuntimeError("Detected columns appear to contain headers rather than data. Please specify columns explicitly using --seller-col, --fee-col, --labels-col arguments.")
...
def filter_header_rows(df, seller_col, enabled=True):
    """Remove rows that appear to be headers rather than data"""
    if not enabled or seller_col not in df.columns:
        return df
    
    # Create a mask for rows that are NOT headers
    mask = ~df[seller_col].apply(lambda x: is_likely_header_value(x, seller_col))
    
    # Keep rows where seller column doesn't look like a header
    filtered_df = df[mask].copy()
    
    if len(filtered_df) < len(df):
        log(f"Filtered out {len(df) - len(filtered_df)} header rows from main data", True)
    
    return filtered_df

def aggregate_main(df, seller_col, seller_name_col, fee_col, labels_col, filter_headers=True):
    df=df.copy(); df.columns=[str(c).strip() for c in df.columns]
    
    # Filter out header rows
    df = filter_header_rows(df, seller_col, filter_headers)
...
def resolve_admin(df, seller_letter=None, storage_letter=None, pim_letter=None, seller_col=None, storage_col=None, pim_col=None):
    ...
    # Validate admin seller column
    if seller and seller in df.columns:
        sample_values = df[seller].dropna().head(5)
        header_count = sum(1 for val in sample_values if is_likely_header_value(val, seller))
        
        if header_count >= len(sample_values) * 0.6:  # If 60%+ look like headers
            log(f"Warning: Admin seller column '{seller}' appears to contain headers rather than seller data", True)
            raise RuntimeError("Admin seller column appears to contain headers. Please specify --admin-seller-col explicitly.")
...
def aggregate_admin(df, admin_seller_col, storage_col, pim_col, filter_headers=True):
    d=df.copy(); d.columns=[str(c).strip() for c in d.columns]
    
    # Filter out header rows
    d = filter_header_rows(d, admin_seller_col, filter_headers)
...
def main():
    ap = argparse.ArgumentParser(description="v3.6.5 strict: header detection and dedup fix")
    ...
    ap.add_argument("--no-header-filter", action="store_true", help="Disable automatic header row filtering")
    ...
    totals, rows_used, issues_rows, issues_summary = aggregate_main(main_df, seller_col, seller_name_col, fee_col, labels_col, not args.no_header_filter)
    ...
    admin_agg = aggregate_admin(admin_df, a_seller, a_storage, a_pim, not args.no_header_filter)
