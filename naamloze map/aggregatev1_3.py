...
def load_admin_table_row3(path, encoding=None, verbose=False, sheet_name=None):
    """Read admin file with header on Excel row 3 (0-based header=2)."""
    ext = os.path.splitext(path)[1].lower()
    hdr = 2  # row 3
    if ext == ".csv":
        # Use Python engine to allow automatic delimiter inference when sep=None
        df = pd.read_csv(path, encoding=encoding or "utf-8-sig", sep=None, engine="python", header=hdr)
        return df, "Sheet1"
    elif ext in [".xlsx", ".xlsm", ".xls"]:
...
def main():
    ...
    admin_df, _ = load_admin_table_row3(args.admin, encoding=args.encoding, verbose=args.verbose, sheet_name=args.admin_sheet)
